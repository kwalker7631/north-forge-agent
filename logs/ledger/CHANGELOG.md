...
- **CHG-2026-09-12-003** — `scripts/build-handoff-bundle.ps1` (new): fixes the handoff-bundle self-reference issue (a report can't contain the real sha256 of a zip that contains that same report — the hash doesn't exist until the zip is sealed). Takes `-SessionReportPath` (used only to write a cross-reference entry — name/size/sha256 — into a generated `HANDOFF-INDEX.md`, never copied into the zip) and `-Sources` (prior sessions' artifacts to actually bundle); drops `-SessionReportPath` from `-Sources` with a warning if a caller passes it there by mistake. **Not verified by running it this session** — a safety classifier began blocking every command-execution action (PowerShell and Bash both) partway through this task, unrelated to this script's content; flagged plainly in `D:\logs\ARCHITECTURE_PLACEMENT_AND_HANDOFF_FIX_2026-09-12.md` rather than claimed as tested. Needs an actual run (by Kenneth, or a future session) before this is a confirmed fix rather than a reviewed-by-reading one. Paths: `scripts/build-handoff-bundle.ps1`. Run: RUN-2026-09-12-002.

### Fixed
- **CHG-2026-09-12-008** — `scripts/build-handoff-bundle.ps1`: fixed the missing-standalone-report handoff gap. The zip correctly never contains the current session's own report (self-hash impossibility, `CHG-2026-09-12` earlier), but that report was only ever written to `logs\`, a different folder from the drive root the zip lands in - in practice the owner repeatedly uploaded only the zip to the primary GPT and forgot the separate report, so actual findings didn't reach it, more than once. Not fixed by reminding harder: the script now also copies `-SessionReportPath` to `<DriveRoot>\<same filename>` (skipped, not an error, if it's already there), and prints an explicit "IMPORTANT: attach BOTH files ... 2. `<report>` <-- the actual findings are in this one" block naming both real filenames right before exit. 5 new tests (`tests/test_build_handoff_bundle.py`) - copy-alongside, zip-still-never-contains-the-report (regression guard for the original fix), same-path no-op, the reminder text itself, and overwriting a stale same-named leftover. This session's own two handoff-bundle builds (RUN-2026-09-12-005, `-006`) predate this fix - the first real test of it is the report this change is filed under. Paths: `scripts/build-handoff-bundle.ps1`, `tests/test_build_handoff_bundle.py`. Run: RUN-2026-09-12-007.

### Added
- **CHG-2026-09-12-007** — `scripts/make-how-to-start-shortcut.ps1` (new) + one line in `north-forge.cmd`: a standing, always-clickable drive-root entry point back to onboarding content ("How To Start.lnk", distinct from "Start North Forge.lnk"), rewritten every launch the same self-healing way `make-drive-root-shortcut.ps1` already handles the agent launcher. Investigated first, per the owner's ask: confirmed nothing standing already existed - `WELCOME.html` was only ever opened by the retired `archive/legacy-standalone-launcher/` path, and `HOW_TO_START.txt` (an EXCALIBUR.md-documented file) is not created by any current script, only checked for as a marker name in `pinokio_lab_target.py`. Resolves the docs-index target generically (chassis-root `WELCOME.html` first, else the first one found under `private-editions/*/`) rather than hardcoding the Kyocera edition, and skips/removes itself gracefully if none exists rather than failing the launch. Separately flagged, not fixed here (Zone B content in `north-forge-hermes-edition`, not this repo): the Kyocera edition's own `WELCOME.html` still describes the retired `launch-north-forge.bat`/`.sh` launcher, not the current drive-native `north-forge.cmd` flow - the new shortcut will faithfully open a stale page until that content is corrected by its own repo's governance. Tests: `tests/test_make_how_to_start_shortcut.py` (6 new, all passing) + reran the existing launcher-hardening/preflight suites (23 passed, 2 skipped, unchanged) to confirm the one-line `north-forge.cmd` addition didn't regress anything. Paths: `scripts/make-how-to-start-shortcut.ps1`, `north-forge.cmd`, `tests/test_make_how_to_start_shortcut.py`. Run: RUN-2026-09-12-006.

### Fixed
- **CHG-2026-09-12-006** — `scripts/lib/report_completeness.py` + `logs/ledger/reports/REPORT-MANIFEST.md`: added a `lost` `Source-Of-Truth` disposition, distinct from `ledger-only`, for a manifest row whose report *was* produced (source was `in-bundle`, its only copy) but that copy was later destroyed by a documented event — counted as covered but always surfaced as a WARN, never silently folded into a routine OK. Applied it to all 30 rows (`RUN-2026-09-06-001` through `RUN-2026-09-11-002`) that `scripts\collect-logs.ps1`'s completeness check was FAILing this session: confirmed by filename search across all of `D:\`, `Downloads\`, and every `*.zip` on both (including every `HANDOFF_*.zip`) that none of the 30 named reports exist anywhere — their sole copies were destroyed by the `D:` wipe already recorded in passing at `RUN-2026-09-11-001`. No content was fabricated or reconstructed for any of them. Also refreshed `RUN-2026-09-12-004`'s stale sha256 (the report was edited after the manifest row was last written). Result: `collect-logs.ps1` went from INCOMPLETE (31 FAIL, 1 WARN) to COMPLETE (0 FAIL, 31 WARN — the 30 cited losses plus none new). `tests/test_report_completeness_handoff_line.py` unchanged, still 12/12 passing. Paths: `scripts/lib/report_completeness.py`, `logs/ledger/reports/REPORT-MANIFEST.md`. Run: RUN-2026-09-12-005.
- **CHG-2026-09-12-005** — `scripts/build-handoff-bundle.ps1`'s default `-DriveRoot` used `Split-Path -Qualifier $SessionReportPath` (a bare `"D:"`), which Windows treats as drive-relative — `Resolve-Path` silently resolved it to whatever directory the process was `cd`'d into on that drive rather than the actual root. First real run (this session, from `D:\north-forge-agent`) landed the bundle at `D:\north-forge-agent\HANDOFF_....zip` instead of the drive root, defeating the point of the convention. Fix: append the path separator so the rooted interpretation is forced; re-verified with a real run from an unrelated `cwd` (`C:\`). Paths: `scripts/build-handoff-bundle.ps1`. Ref: ERR-2026-09-12-002. Run: RUN-2026-09-12-003.
- **CHG-2026-09-12-004** — `scripts/nf_sync_cron.py`'s `discover_cron_jobs()` passed `skills=[]` (not `None`) to `tools.cronjob_tools.cronjob()` whenever a skill declared no extra `skills:` block — the common case, including `kyocera-research`/`daily-brief`. `tools.cronjob_job_args._canonical_skills()` only falls back to the singular `skill=` kwarg when `skills is None`, so the primary skill was silently dropped: every job `nf_sync_cron.py` registered persisted with `"skill": null` and **no skill loaded at all**, not just a cosmetic null field. Found via this session's fresh-verification spot-check of Perplexity's cron/gateway handoff (Part 2 recommendation #1). Fix: fold the resolved primary skill into the `"skills"` list itself so the persisted record never depends on that None-vs-`[]` distinction. Verified end-to-end (no mocking) against the real `cronjob()` with `north-forge-hermes-edition`'s actual `kyocera-research/SKILL.md` in a fresh sandboxed `HERMES_HOME`, before and after. Paths: `scripts/nf_sync_cron.py`, `tests/scripts/test_nf_sync_cron.py` (+2 tests, 17 total, all passing). Ref: ERR-2026-09-12-001. Run: RUN-2026-09-12-003.
